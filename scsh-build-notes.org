...
You can install it in /usr/local with "make install".  I like to keep all of /usr/local writable by my user id, but if it's limited to writing by root, you'll need something like "sudo make install".
...
It can then be installed in /usr/local with "make install" or "sudo make install".
